...
import { Settings as SettingsIcon, User, Shield, Terminal, Cpu, Globe, RefreshCw, LogOut, Wifi, Database, Key, Activity, HeartPulse, HardDrive, Bell } from 'lucide-react';
              ...
              {activeTab === 'profile' && (
                <div className="space-y-12 animate-in slide-in-from-right-4 duration-500">
                   <div className="flex items-center gap-8">
                      ...
                      <div>
                         <h3 className="text-3xl font-black text-white uppercase italic tracking-tight">{user?.name || 'Anonymous User'}</h3>
                         <p className="text-indigo-400 font-bold italic">{user?.email || 'sovereign@node.local'}</p>
                      </div>
                   </div>
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-12 border-t border-white/5">
                      <div className="space-y-3">
                         <label className="text-[9px] font-black text-gray-700 uppercase tracking-widest ml-2">Node Alias</label>
                         <input type="text" defaultValue={user?.name} className="w-full bg-black border border-white/10 rounded-2xl p-5 text-white font-bold outline-none focus:ring-2 focus:ring-indigo-500" />
                      </div>
                      <div className="space-y-3">
                         <label className="text-[9px] font-black text-gray-700 uppercase tracking-widest ml-2">Notification Relay</label>
                         <div className="flex items-center justify-between p-5 bg-black border border-white/10 rounded-2xl">
                            <span className="text-sm font-bold text-gray-500 italic">Flash Traffic</span>
                   ...
                   <button className="px-10 py-5 bg-white text-black font-black rounded-[32px] text-xs uppercase tracking-widest hover:bg-indigo-600 hover:text-white transition-all shadow-xl active:scale-95">Commit Identity Changes</button>
                </div>
              )}
